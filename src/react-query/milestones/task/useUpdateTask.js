...
export const useUpdateTask = (handleClose) => {
  ...
  const { mutate, isLoading } = useMutation(updateTask, {
    ...
    onSuccess: async (newData, localData, context) => {
      ...
      const { milestoneId, orgId, taskId, moduleId, parentId } = localData;
      try {
        if (parentId) {
          for (let i = 0; previousSubTaskCopy?.tasks.length; i++) {
    ...
    onError: (err, localData, context) => {
      ...
      const { milestoneId, orgId, projectId, assigneesStatus, parentId } =
        localData;
      if (parentId) {
        queryClient.setQueryData(
          ["subTask", orgId, milestoneId, parentId],
  ...
  for (let j = 0; tasks?.length; j++) {
    if (tasks[j]?._id === taskId) {
      if (assigneeUpdate) {
        ...
        prevStatus = tasks[j].status;
        let tempAssigneesStatus = tasks[j]?.assigneesStatus?.map((item) => {
      ...
      if (milestones[i]?._id === milestoneId) {
        if (tempStatus === prevStatus || !tempStatus) {
          ...
          milestones[i].taskCount = {
            ...
            [tempStatus]: (milestones[i].taskCount?.[tempStatus] ?? 0) + 1,
          };
        } else {
          milestones[i].taskCount = {
            [tempStatus]: 1,
