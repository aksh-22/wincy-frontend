...
function AddTransaction({ handleClose, orgId, invoiceId, refetch }) {
    ...
    const onValidate = () => {
        ...
        if (!gatewayTransactionId?.trim()?.length) {
            ...
            tempError = {
                ...tempError,
                gatewayTransactionId:
                    'Gateway transaction id field is required.',
            };
        }
        ...
        if (!localEquivalentAmount) {
            ...
            tempError = {
                ...tempError,
                localEquivalentAmount:
                    'Local equivalent amount field is required.',
            };
        }
        ...
        if (!(Number(localEquivalentAmount) > 0) && localEquivalentAmount) {
            ...
            tempError = {
                ...tempError,
                localEquivalentAmount:
                    'Local equivalent amount must be greater than zero.',
            };
        }
    ...
    const onSubmit = () => {
        if (!onValidate()) {
            let obj = JSON.parse(
                JSON.stringify(details, (k, v) => v || undefined)
            );
            console.log('obj', obj);
            obj = {
                ...obj,
            ...
            addInvoiceTransaction({
                ...
                .then((res) => {
                    ...
                    let transactionList = query.getQueryData([
                        'transaction',
                        orgId,
                        invoiceId,
                    ]);
                    ...
                    let viewInvoice = query.getQueryData([
                        'invoiceView',
                        orgId,
                        invoiceId,
                    ]);
                    console.log(
                        ...
                        Number(viewInvoice.paidAmount || 0) +
                            Number(res?.transaction?.amount || 0)
                    );
                    viewInvoice = {
                        ...viewInvoice,
                        paidAmount:
