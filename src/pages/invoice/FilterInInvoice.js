                  ...
                  <div>
                    <li
                      onClick={() => {
                        setFilter({
                          ...filter,
                          subsiduary : filter?.subsiduary === item?._id ? null :item?._id
                        })
                      }}
                      className={`${css.tile}  ${index > 0 ? "mt-5px" : ""} ${
                        subsidiaryList.length - 1 === index ? "mb-1" : ""
                      } ${
                        filter?.subsiduary === item?._id ? css.activeTile : ""
                      }`}
                    >
                      <div
                      ...
                      >
                        <Checkbox
                          checked={filter?.subsiduary === item?._id}
                          size="small"
                          sx={{
                            color: "#c8c8d0",
                            "&.Mui-checked": {
...
setFilter({
  ...filter,
  financialYear : filter?.financialYear?._id === item?._id ? null :item
})
                  }}
                  className={`${css.tile}  ${index > 0 ? "mt-5px" : ""} ${
                    financialYearArray?.length - 1 === index ? "mb-1" : ""
                  } ${filter?.financialYear?._id === item?._id ? css.activeTile : ""}`}
                >
                  <div
                    className={css.textColumn}
                    // style={{ fontSize: "1rem" }}
                  >
                    <Checkbox
                      checked={filter?.financialYear?._id === item?._id}
                      size="small"
                      sx={{
                        color: "#c8c8d0",
                        "&.Mui-checked": {
...
setFilter({
  ...
  sortBy :"asc"
              ...
              className={`${css.tile}  mb-1 ${filter?.sortBy === "asc" ? css.activeTile : ""}`}
            >
              <div
                className={css.textColumn}
                // style={{ fontSize: "1rem" }}
              >
                ...
                />
                <p className="filter_title">Ascending (default)</p>
              </div>
            </li>
            ...
            <li
              ...
              className={`${css.tile}  mb-1 ${filter?.sortBy === "desc" ? css.activeTile : ""}`}
            >
              {" "}
              <div
                className={css.textColumn}
...
return `${moment(endYear , "YYYY").format("YY")}-${moment(startYear , "YYYY").format("YY")}`
}
