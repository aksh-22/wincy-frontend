...
function SubTaskInfoSidebar({
    ...
    const { mutate: mutateUpdateTask } = useUpdateTask(undefined);
    const { mutate: mutateUpdateTaskDescription } = useUpdateTask(
        undefined,
        true
    );
    ...
    const { mutate: markMutate } = useMarkMultipleSubtask(
        fromModule,
        projectId
    );
    ...
    return (
        <>
            ...
            <CustomTextEditor
                ...
                // disable={disabled}
            />
            {taskInfo.descriptionUpdatedAt ? (
                <div style={{ margin: '30px 0', display: 'flex' }}>
                    <p style={{ color: 'var(--gray)', marginRight: 10 }}>
                        Description last updated at :-
                    </p>
                    <p style={{ color: 'var(--gray)' }}>
                        {moment(taskInfo.descriptionUpdatedAt).format(
                            'DD / MM / YY - hh:mm a'
                        )}
                    </p>
                </div>
            ...
            <div style={{ height: 'inherit' }} className='mt-2'>
                ...
                <div className={`${css.tableContainer} mb-2`}>
                    <CustomRow
                        value={moment(taskInfo?.createdAt).format(
                            'DD-MMM-YYYY, hh:MM A'
                        )}
                        // onChange={onValueChange}
                        inputType='text'
