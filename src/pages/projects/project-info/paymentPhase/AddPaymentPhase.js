...
function AddPaymentPhase({ onClose }) {
    ...
    const { orgId, projectId, project } = useProjectTeam();
    ...
    console.log('project', project.paymentInfo.currency);

    const [details, setDetails] = useState({
        title: '',
        description: '',
        currency: project.paymentInfo.currency,
        amount: '',
        restricted: false,
    });
    ...
    useEffect(() => {
        if (paymentPhaseList && data) {
            let tempMilestone = paymentPhaseList?.filter(
                (item) => item?.milestoneIds?.length > 0
            );
            tempMilestone = tempMilestone?.map((item) =>
                item?.milestoneIds?.map((row) => row)
            );
            tempMilestone = tempMilestone?.flat();
            let tempMilestoneList = data[3]?.milestonesData?.filter(
                (item) => !tempMilestone?.includes(item?._id)
            );
    ...
    const onSubmit = () => {
        const { title, currency, amount, description, restricted } = details;
        let tempError = error;
        let isError = false;
            ...
            .then((res) => {
                ...
                selectedMilestone?.length > 0 &&
                    ...
                    updatePaymentPhaseMilestone({
                        ...
                        projectId,
                        data: {
                            newMilestones: selectedMilestone?.map(
                                (item) => item?._id
                            ),
    ...
    const onRestrictCHange = () => {
        setDetails((prev) => ({ ...prev, restricted: !prev.restricted }));
    };
        ...
        <div>
            ...
            {userType.userType === 'Admin' ? (
                <div className='alignCenter my-2'>
                    <CheckBoxSquare
                        onClick={onRestrictCHange}
                        isChecked={details.restricted}
                        color='var(--chipRed)'
                    />
                    {/* <Switch
                    checked={details.restricted}
                    onChange={onRestrictCHange}
                /> */}
                    <p className='mx-1'>Restrict invoice for other users</p>
                </div>
            ) : null}
