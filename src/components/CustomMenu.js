...
function CustomMenu({
  ...
  const handleClose = (item, outside) => {
    item?.value === "Completed" ? setIsCompleted(true) : setIsCompleted(false);
    if (outside) {
      setAnchorEl(null);
      if (isSelected) {
    ...
    if (multiple) {
      if (isSelected.filter((value) => value === item?.value).length === 0) {
        item?.value && setIsSelected([...isSelected, item?.value]);
      } else {
        setIsSelected(isSelected.filter((value) => value !== item?.value));
      }
    } else {
      if (item?.label) {
        setTimeout(() => {
  ...
  return (
    ...
    {actionButton ? <div onClick={(e) => !disabled && handleClick(e)}>{actionButton} </div> :  <LightTooltip 
      // placement="top"
      arrow title={disabled ? ((Array.isArray(activeMenuItem) && !activeMenuItem?.length) ? "Platform not assigned yet!" : "") : ""}>
        <div
          className={`inheritParent d_flex alignCenter justifyContent_center  `}
          style={{
            cursor: disabled ? "default" : "pointer",
            position: "relative",
            backgroundColor: getBackgroundColor({ label: activeMenuItem }),
            overflow: "hidden",
            ...style
          }}
        ...
        >
          ...
          {Array.isArray(activeMenuItem) ? (
            !isSelected.length ? (
              <LightTooltip title={disabled ? "" : "Click to add platform"} arrow>
                <div>
                  <Icon name="platform" />
                </div>
              </LightTooltip>
              ...
              menuName === "bugStatus" ? (
                <div
                  ...
                  style={{
                    ...
                    borderBottom:
                      menuItems?.length - 1 === i ? "" : "1px solid gray",
                  }}
                >
