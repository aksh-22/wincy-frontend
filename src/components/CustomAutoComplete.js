    ...
    classes={{ paper: classes.paper }}
      ...
      renderTags={(tags) => {
        ...
        return tags?.map((tag) => (
          <CustomChip
            label={tag.name}
            id={tag._id}
            avatar={<CustomAvatar small src={tag.profilePicture ?? null} />}
      ...
      renderOption={(option) => (
        ...
        >
          {<CustomAvatar
          variant="circle"
          src={option?.profilePicture ?? null} small />}
          <p style={{ fontSize: 14, marginLeft: 5 , color:"var(--defaultWhite)"}}>{option?.name}</p>
        </div>
      )}
      renderInput={(params) => (
        <TextField
